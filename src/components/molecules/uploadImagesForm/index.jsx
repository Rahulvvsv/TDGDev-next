...
const UploadForm = () => {
  ...
  const handleSubmit = async (e) => {
      e.preventDefault();
    const google_recaptcha = await fetch("/api/uploadImage", {
      method: "POST",
      headers: {
      ...
      body: JSON.stringify({
        gRecaptchaToken: gtoken,
      }),
    });

    let data = await google_recaptcha.json();
    console.log(data);
      ...
      {!submitted ? (
        ...
        >
          ...
          <h1 className={style.heading1}>
            Thank you for your generous offer to donate your Furniture. We will
